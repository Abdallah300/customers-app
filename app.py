...
# ================== 5. واجهة الإدارة (إدارة العملاء كاملة) ==================
if st.session_state.role == "admin":
    ...
    if menu == "👥 إدارة العملاء":
        ...
        if search:
            ...
            for c in filtered:
                ...
                with col1:
                    ...
                    # --- رجوع بيانات العميل والـ GPS هنا ---
                    with st.expander("📍 تعديل بيانات العميل والـ GPS"):
                        c['name'] = st.text_input("الاسم", c['name'], key=f"un{c['id']}")
                        c['phone'] = st.text_input("التليفون", c.get('phone',''), key=f"up{c['id']}")
                        c['gps'] = st.text_input("رابط موقع GPS", c.get('gps',''), key=f"ug{c['id']}")
                        if st.button("حفظ التعديلات", key=f"us{c['id']}"):
                            save_json("customers.json", st.session_state.data)
                            st.success("تم حفظ البيانات بنجاح")
                
                with col2:
                    with st.expander("💸 تسجيل صيانة أو تحصيل"):
                        d1 = st.number_input("تكلفة صيانة (+)", key=f"d{c['id']}"); d2 = st.number_input("مبلغ محصل (-)", key=f"r{c['id']}")
                        note = st.text_input("بيان العملية", key=f"nt{c['id']}")
        ...
        with st.form("new_cust"):
            n = st.text_input("اسم العميل الجديد"); p = st.text_input("التليفون"); loc = st.text_input("رابط GPS"); d = st.number_input("مديونية سابقة")
            if st.form_submit_button("إضافة العميل"):
                ...
                save_json("customers.json", st.session_state.data); st.success(f"تمت الإضافة! كود العميل: {new_id}")
...
# ================== 6. واجهة الفني ==================
    ...
    if target.get('gps'): st.link_button("📍 فتح موقع العميل على الخريطة", target['gps'])
    with st.form("tech_visit"):
        v_d = st.number_input("تكلفة الصيانة/الشمع", 0.0); v_p = st.number_input("المبلغ المحصل", 0.0); v_n = st.text_area("تفاصيل ما تم عمله")
        if st.form_submit_button("إرسال التقرير"):
            ...
            save_json("customers.json", st.session_state.data); st.success("تم إرسال التقرير بنجاح")

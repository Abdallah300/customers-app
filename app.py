        ...
        if c:
            ...
            if c.get('history'):
                ...
                for h in reversed(history_with_balance):
                    ...
                    st.info(f"💰 المديونية المتبقية بعد العملية: {h['after_bal']:,.0f} ج.م")
            else:
...
if st.session_state.role == "admin":
    ...
    if menu == "👥 إدارة العملاء":
        ...
        if "confirm_delete" in st.session_state:
            ...
            st.warning("⚠️ هل أنت متأكد من الحذف النهائي؟ لا يمكن التراجع")
    ...
    # ================== الفنيين ==================
        ...
        # ====== ADD : TECH TRACKING ======
        st.subheader("📊 متابعة شغل الفنيين")
                ...
                for c in st.session_state.data:
                    for h in c.get('history', []):
                        if h.get('tech') == t['name']:
                            st.markdown(
                                f"👤 {c['name']} | 📝 {h['note']} | 🔴 {h['debt']} | 🟢 {h['price']} | 📅 {h['date']}"
                            )
...
# ================== 6. واجهة الفني ==================
    ...
    with st.form("tech_visit"):
        ...
        v_add = st.number_input("تكلفة / مديونية", min_value=0.0)

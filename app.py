...
import matplotlib.pyplot as plt  # إضافة جديدة
...
<style>  
    ...
    * { font-family: 'Cairo', sans-serif; text-align: right; direction: rtl; }  
    ...
    /* إضافة جديدة: تنسيق للأرقام المهمة */
    .highlight-number {
        font-size: 1.5em;
        color: #00ffcc;
        font-weight: bold;
    ...
    try:
        ...
        if c:
            ...
            if c.get('history'):  
                ...
                # إضافة جديدة: ملخص بياني (إذا كان هناك بيانات كافية)
        ...
        # إضافة جديدة: زر للطباعة والتقرير
        st.markdown("---")
...
if "role" not in st.session_state:
    ...
    # إضافة جديدة: إحصائيات سريعة على شاشة الدخول
...
# (منطق تسجيل الدخول - لم يتم حذف شيء - نفس الكود الأصلي)
...
if st.session_state.role == "admin":
    ...
    # إضافة جديدة: إحصائيات في الشريط الجانبي
    st.sidebar.markdown("---")
    total_balance = sum(calculate_balance(c.get('history', [])) for c in st.session_state.data)
    ...
    if menu == "👥 إدارة العملاء":  
        # إضافة جديدة: حقل بحث متقدم
        col_search1, col_search2 = st.columns(2)
        with col_search1:
            search_name = st.text_input("بحث بالاسم...", key="search_name")
        with col_search2:
            search_gov = st.selectbox("فلترة بالمحافظة", 
                                    ["جميع المحافظات"] + list(set(c.get('gov', '') for c in st.session_state.data if c.get('gov'))),
                                    key="search_gov")
        
        filtered_data = st.session_state.data
        if search_name:
            filtered_data = [c for c in filtered_data if search_name.lower() in c['name'].lower()]
        ...
        for i, c in enumerate(filtered_data):  
            ...
            # إضافة جديدة: تلوين حسب المديونية
            balance_color = "#ff4444" if balance > 0 else "#44ff44" if balance < 0 else "#888888"
            
            with st.expander(f"👤 **{c['name']}** | الرصيد: <span style='color:{balance_color}'>{balance:,.0f} ج.م</span> | المحافظة: {c.get('gov', '---')}", unsafe_allow_html=True):  
                ...
                with st.form(f"adm_f_{c['id']}"):  
                    c['gov'] = st.text_input("المحافظة", value=c.get('gov', ''), key=f"gov_{c['id']}")  
                    c['branch'] = st.text_input("الفرع", value=c.get('branch', ''), key=f"branch_{c['id']}")  
                    
                    col_form1, col_form2 = st.columns(2)
                    with col_form1:
                        a_add = st.number_input("إضافة مديونية (+)", min_value=0.0, key=f"add_{c['id']}")  
                    with col_form2:
                        a_rem = st.number_input("خصم مبلغ (تحصيل) (-)", min_value=0.0, key=f"rem_{c['id']}")  
                    
                    note = st.text_input("بيان العملية", value="تسويه إدارية", key=f"note_{c['id']}")  
                    ...
                    with col_btn1:
                        if st.form_submit_button("💾 حفظ التعديلات", use_container_width=True):  
                            if a_add > 0 or a_rem > 0:  
                                ...
                                create_backup()  # إضافة النسخ الاحتياطي
                            save_json("customers.json", st.session_state.data)
                            st.success("تم الحفظ بنجاح!")
                            st.rerun()
                    with col_btn2:
                        if st.form_submit_button("🗑️ حذف العميل", use_container_width=True):
                            if st.checkbox(f"تأكيد حذف العميل {c['name']}?", key=f"confirm_del_{c['id']}"):
                ...
                # إضافة جديدة: أزرار إضافية
                col_actions1, col_actions2, col_actions3 = st.columns(3)
                with col_actions1:
                    if st.button("🖼️ باركود", key=f"qr_{c['id']}", use_container_width=True):  
                        st.image(f"https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=https://customers-app-ap57kjvz3rvcdsjhfhwxpt.streamlit.app/?id={c['id']}")  
                with col_actions2:
                    if st.button("📋 سجل العمليات", key=f"history_{c['id']}", use_container_width=True):
                        st.write(f"**سجل عمليات {c['name']}:**")
                        for h in reversed(c.get('history', [])):
                            st.write(f"- {h['date']}: {h['note']} ({h['tech']})")
                with col_actions3:
                    if st.button("📧 مشاركة", key=f"share_{c['id']}", use_container_width=True):
                        st.info(f"رابط العميل: https://customers-app-ap57kjvz3rvcdsjhfhwxpt.streamlit.app/?id={c['id']}")
        ...
        # إضافة جديدة: جدول بأكبر المدينين
        st.subheader("📋 أكبر 10 مدينين")
        customers_with_balance = sorted(
            [(c['name'], calculate_balance(c.get('history', []))) for c in st.session_state.data],
            key=lambda x: x[1],
            reverse=True
        )[:10]
        ...
        # إضافة جديدة: تحسين واجهة إدارة الفنيين
        ...
        with col_tech1:
            with st.form("add_tech"):
                ...
                if st.form_submit_button("➕ إضافة فني"):
                    new_tech = {"name": tech_name, "pass": tech_pass, "phone": tech_phone}
        ...
        with col_tech2:
            ...
            for tech in st.session_state.techs:
                with st.expander(f"🛠️ {tech['name']}"):
                    st.write(f"كلمة السر: {tech['pass']}")
                    if tech.get('phone'):
                        st.write(f"الهاتف: {tech['phone']}")
                    if st.button(f"🗑️ حذف {tech['name']}", key=f"del_tech_{tech['name']}"):
                        st.session_state.techs.remove(tech)
                        save_json("techs.json", st.session_state.techs)
                        st.rerun()

    elif menu == "📁 النسخ الاحتياطية":  # إضافة جديدة
        ...
        if os.path.exists(backup_dir):
            ...
            for backup_file in backup_files:
                ...
                with col1:
                    st.write(backup_file.replace('.json', '').replace('customers_backup_', ''))
                with col2:
...
================== 6. واجهة الفني الكاملة ==================
    ...
    # إضافة جديدة: عرض معلومات العميل المختار
    if target:
        current_balance = calculate_balance(target.get('history', []))
        st.info(f"**الرصيد الحالي:** {current_balance:,.0f} ج.م")
    ...
    # إضافة جديدة: تاريخ زيارات الفني للعميل
    if target:
        ...
        if tech_visits:
            for visit in reversed(tech_visits):
                st.write(f"**{visit['date']}** - {visit['note']}")
                if float(visit.get('debt', 0)) > 0:
                    st.write(f"  ↳ تكلفة: {float(visit.get('debt', 0)):,.0f} ج.م")
                if float(visit.get('price', 0)) > 0:
                    st.write(f"  ↳ محصل: {float(visit.get('price', 0)):,.0f} ج.م")
                st.write("---")
        else:

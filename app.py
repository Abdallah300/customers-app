...
# دالة حساب المديونية الصافية (إجمالي الديون المضافة - إجمالي المبالغ المسددة)
...
if "id" in params:
    try:
        ...
        if customer:
            ...
            col2.metric("⚠️ المديونية المتبقية", f"{balance:,.0f} ج.م")
            st.markdown(f"<div class='client-report'><div class='data-row'>👤 العميل: <b>{customer.get('name')}</b></div><div class='data-row'>🏛️ الفرع: <b>{customer.get('branch')}</b></div><div class='data-row'>🔧 الجهاز: <b>{customer.get('device_type')}</b></div></div>", unsafe_allow_html=True)
            st.stop()
    except: pass
...
# (منطق دخول الإدارة والفني كما هو في الكود السابق)
...
if st.session_state.role == "tech_login":
    ...
    t_user = st.selectbox("اسمك", t_list)
    p = st.text_input("السر", type="password")
    if st.button("دخول"):
...
# ================== 5. واجهة الفني (الحسابات الصحيحة) ==================
if st.session_state.role == "tech":
    ...
    if t_menu == "📋 قائمة العملاء":
        ...
        with st.form("tech_f"):
            paid = st.number_input("المبلغ اللي استلمته من العميل (يقلل المديونية)", min_value=0.0)
            added = st.number_input("تكلفة الصيانة أو الجهاز الجديد (يزود المديونية)", min_value=0.0)
...
# ================== 6. واجهة الإدارة (إضافة عميل وحسابات صحيحة) ==================
    ...
    if menu == "👥 إدارة العملاء":
        ...
        with st.form("add_client"):
            ...
            loc = st.text_input("العنوان التفصيلي")
            dev = st.selectbox("نوع الجهاز", ["جديد", "خارجي", "قديم"])
            init_debt = st.number_input("المديونية الافتتاحية (إن وجد)", min_value=0.0)
        ...
        total_market_debt = sum(calculate_balance(c.get('history', [])) for c in st.session_state.data)
        st.metric("صافي مديونيات السوق (لك عند الناس)", f"{total_market_debt:,.0f} ج.م")
